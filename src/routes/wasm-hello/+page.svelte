			...
			function setupInteraction(canvas) {
				...
				function onMouseClick(event) {
					...
					if (intersects.length > 0) {
						...
						const gridPos = [Math.round(position.x), Math.round(position.y), Math.round(position.z)];
						...
						const isObstacle = obstacles.some(
							(obs) => obs[0] === gridPos[0] && obs[1] === gridPos[1] && obs[2] === gridPos[2]
						);
...
<p class="description">
	...
	<br /><strong>Controls:</strong> Click grid cells to move start/goal points. Press SPACE to toggle between
